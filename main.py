...
def create_jira(project, issuetype, summary, assignee=None, description=None, parent=None, components=None, priority=None, estimate=None):
    ...
    _issue.update(fields={"timetracking": {"originalEstimate": "1w"}})
    return _issue
        ...
        if color in VALIDATION_COLOR:
            ...
            if key_cell.value is None:
                ...
                if issuetype_cell.value == "Epic":
                    ...
                    new_issue = create_jira(PROJECT, issuetype_cell.value, summary_cell.value,
                                            description=description_cell.value, assignee=assignee_cell.value,
                                            priority=priority_cell.value, parent=last_epic)
                    ...
                    if last_epic is not None:
                        new_issue = create_jira(PROJECT, issuetype_cell.value, summary_cell.value,
                                                description=description_cell.value, assignee=assignee_cell.value,
                                                priority=priority_cell.value, parent=last_epic, estimate=estimate_cell.value)
                    else:
                        new_issue = create_jira(PROJECT, issuetype_cell.value, summary_cell.value,
                                                description=description_cell.value, assignee=assignee_cell.value,
